 ...
 * corresponde con un registro en la tabla <b>document_reference</b> de la base de datos.
 *
 * @author Juan Antonio
 * @version 1.0
 * @since 04/06/2024
...
public class DocumentReference extends AuditableCreatedAt {
  ...
  @Column(name = "document_reference_id", nullable = false, length = TamanoCampos.TAMANO_500)
  private String documentReferenceId;

  @Column(name = "document_type_code", length = TamanoCampos.TAMANO_50)
  private String documentTypeCode;

  @Column(name = "document_type", length = TamanoCampos.TAMANO_500)
  private String documentType;

  @Enumerated(EnumType.STRING)
  @Column(name = "tipo_documento", length = TamanoCampos.TAMANO_50)
  ...
  @JoinColumn(
      ...
      foreignKey =
          @ForeignKey(
              ...
              foreignKeyDefinition =
                  "FOREIGN KEY (general_document_reference_id) "
                      + "REFERENCES contract_folder_status(id) ON DELETE CASCADE"))
  private ContractFolderStatus generalDocumentReference;

  @ManyToOne(fetch = FetchType.LAZY)
  @JoinColumn(
      name = "pmcs_general_document_reference_id",
      referencedColumnName = "id",
      foreignKey =
          @ForeignKey(
              name = "fk_docref_general_pmcs",
              foreignKeyDefinition =
                  "FOREIGN KEY (pmcs_general_document_reference_id) "
                      + "REFERENCES preliminary_market_consultation_status(id) ON DELETE CASCADE"))
  private PreliminaryMarketConsultationStatus generalDocumentReferencePmcs;
}
