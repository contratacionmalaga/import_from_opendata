 ...
 * <p>Contrato:
 * <ul>
 *   <li>Vincula cada modificación con su {@link ContractFolderStatus} padre.</li>
 *   <li>Los campos string se truncan según {@link Constantes}.</li>
 *   <li>Los subnodos opcionales (monetarios y medidas) se mapean solo si existen.</li>
 * </ul>
 * </p>
 */
public final class MapperContractModification {
  ...
  /**
   * Convierte una lista de modificaciones CÓDICE a lista de entidades
  ...
  public static List<ContractModification> getListContractModificationFromType(
    ...
    Objects.requireNonNull(contractModificationTypeList,
                           "contractModificationTypeList no puede ser null");
  ...
  private static void mapIdentifiers(ContractModification entity, ContractModificationType type) {
    setLimited(entity::setContractId,
               safeValue(type.getContractID(), ContractIDType::getValue)
    );

    setLimited(entity::setContractModificationId,
               safeValue(type.getID(), IDType::getValue)
    );
  }
  ...
  private static void mapNote(ContractModification entity, ContractModificationType type) {
    // Según tu contexto: listas no nulas (al menos vacías). Si note fuese null, el helper debería tolerarlo.
  ...
  private static void mapLegalMonetaryTotals(ContractModification entity, ContractModificationType type) {
    ...
    mapTaxExclusiveAmount(
        type.getFinalLegalMonetaryTotal(),
        entity::setFinalTaxExclusiveAmount
    );
  }
  ...
  private static void mapTaxExclusiveAmount(
    ...
    LegalMonetaryTotal total = LegalMonetaryTotalMapper.getLegalMonetaryTotalFromType(
  ...
  private static void mapMeasures(ContractModification entity, ContractModificationType type) {
    ...
    mapMeasure(
        measureType,
        entity::setFinalMeauseUnitCode,
        entity::setFinalMeasureValue
    );
  }
  ...
  private static void setLimited(
    ...
    setter.accept(ComunHelper.limitarRegistro(value, Constantes.TAMANO_MAXIMO_CAMPO_50));
  }
}
