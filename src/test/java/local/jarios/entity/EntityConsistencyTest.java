...
class EntityConsistencyTest {
    ...
    @Test
    @DisplayName("Relaciones con CascadeType.ALL deben revisarse manualmente")
    void cascadeAllShouldBeReviewed() {
        Reflections reflections = new Reflections(BASE_PACKAGE);
            ...
            for (Field field : entity.getDeclaredFields()) {
                if (field.isAnnotationPresent(OneToOne.class)) {
                    ...
                    if (!rel.mappedBy().isEmpty()) {
                        // Es la parte débil → no debería tener ALL
                        assertFalse(hasCascadeAll(rel.cascade()),
                                () -> entity.getSimpleName() + "." + field.getName() +
                                        " es parte débil (mappedBy) y no debería usar CascadeType.ALL");
                    }
                }
                if (field.isAnnotationPresent(ManyToOne.class)) {
                    CascadeType[] cascades = field.getAnnotation(ManyToOne.class).cascade();
                    assertFalse(hasCascadeAll(cascades),
                            () -> entity.getSimpleName() + "." + field.getName() +
                                    " usa CascadeType.ALL → revisar si es correcto");
                }
                if (field.isAnnotationPresent(OneToMany.class)) {
                    CascadeType[] cascades = field.getAnnotation(OneToMany.class).cascade();
                    assertFalse(hasCascadeAll(cascades),
                            () -> entity.getSimpleName() + "." + field.getName() +
                                    " usa CascadeType.ALL → revisar si es correcto");
                }
                if (field.isAnnotationPresent(ManyToMany.class)) {
                    CascadeType[] cascades = field.getAnnotation(ManyToMany.class).cascade();
                    assertFalse(hasCascadeAll(cascades),
                            () -> entity.getSimpleName() + "." + field.getName() +
                                    " usa CascadeType.ALL → revisar si es correcto");
                }
            ...
            for (Field field : entityClass.getDeclaredFields()) {
                ...
                if (joinColumn != null) {
                    ...
                    if (fkName != null && !fkName.isEmpty()) {
                        String expectedPrefix = "fk_" +
                                entityClass.getSimpleName().toLowerCase() + "_" +
                                field.getName().toLowerCase();

                        assertEquals(
                                fkName, expectedPrefix, () -> "Entidad: " + entityClass.getSimpleName() +
                                ", campo: " + field.getName() +
                                " → FK encontrada: '" + fkName + "' " +
                                "pero se esperaba: '" + expectedPrefix + "'");
                    }
                }
            }
        }
